...
plot.gllvm.quadratic <- function(x, which = 1:5, caption = c(
                                   "Residuals vs linear predictors", "Normal Q-Q", "Residuals vs row index",
                                   "Residuals vs column index", "Scale-Location"
                                 ), var.colors = NULL, add.smooth = TRUE, envelopes = TRUE, reps = 150, envelope.col = c(
                                   "blue",
                                   "lightblue"
                                 ), n.plot = NULL, ...) {
  n <- NROW(x$y)
  ...
  if (1 %in% which) {
    if (is.null(gr.pars$xlim)) {
      plot(eta.mat, ds.res,
        xlab = "linear predictors", ylab = "Dunn-Smyth-residuals", type = "n", col = rep(col, each = n),
  ...
  if (4 %in% which) {
    plot(rep(1:p, each = n), ds.res,
      xlab = "species index", ylab = "Dunn-Smyth-residuals", col = rep(col[csum], each = n), main = mains[4],
      ...
    )
    abline(0, 0, col = "grey", lty = 3)
    if (add.smooth) {
  ...
  if (5 %in% which) {
    ...
    if (is.null(gr.pars$xlim)) {
      plot(eta.mat, sqres, xlab = "linear predictors", ylab = yl, col = rep(col, each = n), main = mains[5], xlim = c(
        min(xxx),
        max(xxx)
      ), ...)
    } else {
