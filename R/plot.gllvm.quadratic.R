...
plot.gllvm.quadratic <- function(x, which = 1:5, caption = c("Residuals vs linear predictors", "Normal Q-Q", "Residuals vs row index", 
    "Residuals vs column index", "Scale-Location"), var.colors = NULL, add.smooth = TRUE, envelopes = TRUE, reps = 150, envelope.col = c("blue", 
    "lightblue"), n.plot = NULL, ...) {
    n <- NROW(x$y)
    p <- NCOL(x$y)
    ...
    if (!is.null(var.colors)) {
        ...
        col <- (1:p)[csum] else col <- (grDevices::rainbow(p + 1)[2:(p + 1)])[csum]
    ...
    if (1 %in% which) {
        if (is.null(gr.pars$xlim)) {
            plot(eta.mat, ds.res, xlab = "linear predictors", ylab = "Dunn-Smyth-residuals", type = "n", col = rep(col, each = n), 
        ...
        if (add.smooth) 
            gamEnvelope(eta.mat, ds.res, col = rep(col, each = n), envelopes = envelopes, envelope.col = envelope.col, ...)
    ...
    if (3 %in% which) {
        plot(rep(1:n, p), ds.res, xlab = "site index", ylab = "Dunn-Smyth-residuals", col = rep(col, each = n), main = mains[3], ...)
        abline(0, 0, col = "grey", lty = 3)
        if (add.smooth) 
            panel.smooth(rep(1:n, p), ds.res, col = rep(col, each = n), col.smooth = envelope.col[1], ...)
        # panel(rep(1:n, p), ds.res, col = rep(col, each = n), cex = 1, cex.lab = 1, cex.axis = 1, lwd = 1)
    }
    if (4 %in% which) {
        plot(rep(1:p, each = n), ds.res, xlab = "species index", ylab = "Dunn-Smyth-residuals", col = rep(col[csum], each = n), main = mains[4], 
            ...)
        abline(0, 0, col = "grey", lty = 3)
        if (add.smooth) 
            panel.smooth(rep(1:p, each = n), ds.res, col = rep(col[csum], each = n), col.smooth = envelope.col[1], ...)
    ...
    if (5 %in% which) {
        ...
        if (is.null(gr.pars$xlim)) {
            plot(eta.mat, sqres, xlab = "linear predictors", ylab = yl, col = rep(col, each = n), main = mains[5], xlim = c(min(xxx), 
                max(xxx)), ...)
        } else {
            plot(eta.mat, sqres, xlab = "linear predictors", ylab = yl, col = rep(col, each = n), main = mains[5], ...)
        }
        if (add.smooth) 
            panel.smooth(eta.mat, sqres, col = rep(col, each = n), col.smooth = envelope.col[1], ...)
