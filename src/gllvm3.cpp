...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_MATRIX(b);
  PARAMETER_MATRIX(B);
  PARAMETER_VECTOR(opt);
  PARAMETER_MATRIX(tol);
  ...
  for (int j=0; j<p; j++){
    for (int i=0; i<num_lv; i++){
      if (j < i){
        ...
        opt_mat(i,j) = opt(j);
        if (i > 0){
          opt_mat(i,j) = opt(i+j+i*p-(i*(i-1))/2-2*i);
        }
      }
      // set diag>0 !!!!!!!!!!!
      if (j == i){
        opt_mat(i,j) = fabs(opt_mat(i,j));
      }
    }
  }
  ...
  matrix <Type> tol_var(num_lv,p);
  //lambda2 = lambda2.cwiseAbs(); //sign constraint quadratic effect
  for (int j=0; j<p; j++){
    for (int i=0; i<num_lv; i++){
  tol_var(i,j) = pow(tol(i,j),2);
    }
  }
  for (int j=0; j<p; j++){
    for (int i=0; i<num_lv; i++){
      newlam2(i,j) = -.5/tol_var(i,j); //positive only
    }
  }
  for (int j=0; j<p; j++){
    for (int i=0; i<num_lv; i++){
      newlam(i,j) = opt_mat(i,j)/tol_var(i,j);
    }
  }
