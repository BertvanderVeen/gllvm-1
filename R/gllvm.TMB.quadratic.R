...
gllvm.TMB.quadratic <- function(y, X = NULL, formula = NULL, num.lv = 2, family = "poisson",
                                ...
                                Lambda.start=c(0.1,0.5), jitter.var=0, start.method="FA", par.scale=1, fn.scale=1, zeta.struc = "species", maxit.lingllvm = NULL, starting.val.lingllvm = "res", equal.tolerances = FALSE, parallel=FALSE,start.struc="species", gamma1=0,gamma2=0,theta4 = NULL, n_int = 7) {
  ...
  makeMod<-function(i){
    ...
    extra <- 0
    if(family == "poisson"&n_int==0) { familyn <- 0}else{familyn=5}
