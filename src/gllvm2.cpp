...
Type objective_function<Type>::operator() ()
{
  ...
  if(method<1){
    ...
    if(rowstruc.size()>1){
      if(random(0)>0){
        for (int i=0; i<n; i++) {
          Ar(rowstruc(i)-1)=pow(Ar(rowstruc(i)-1),2);
          for (int j=0; j<p;j++){
            cQ(i,j) = 0.5* Ar(rowstruc(i)-1);//
          }
        }
      }
      for (int j=0; j<p;j++){
    ...
    if(nlvr>0){
      ...
      if(rowstruc.size()==1)nll -= -0.5*n*log(Cu.determinant())*random(0);//n*
    ...
    if(family==0){
      for (int i=0; i<n; i++) {
        ...
        if(rowstruc.size()>1)nll -= 0.5*(log(Ar(rowstruc(i)-1)) - Ar(rowstruc(i)-1)/pow(sigma,2) - pow(r0(rowstruc(i)-1,0)/sigma,2))*random(0);      }
    } else if(family==1){
      for (int i=0; i<n; i++) {
        for (int j=0; j<p;j++){
    ...
    if(rowstruc.size()>1)nll -= -n*log(sigma)*random(0);// -0.5*t(u_i)*u_i
    ...
    if(random(0)>0 && rowstruc.size()>1){
      for(int i = 0; i < n; i++){
        nll -= dnorm(r0(rowstruc(i)-1,0), Type(0), sigma, true)*random(0);
      }
    }
