...
ordiplot.gllvm.quadratic <- function(object, biplot = FALSE, ind.spp = NULL, alpha = 0.5, main = NULL, which.lvs = NULL,
    ...
    if (length(which.lvs) > 1) {
      ...
      choose.lvs <- do.svd$u * matrix( do.svd$d[1:length(which.lvs)] ^ alpha,
          nrow = n, ncol = length(which.lvs), byrow = TRUE )
      choose.lv.coefs <- do.svd$v * matrix(do.svd$d[1:length(which.lvs)] ^ (1 - alpha),
          nrow = p, ncol = length(which.lvs), byrow = TRUE )
      ...
      if(length(which.lvs)==1){
        ...
        if(any(excl)){
          message(paste("The species",paste(row.names(quadr.coef[excl,]),collapse=", "), "lack a quadratic response on the chosen LV(s) and won't be plotted.",sep=" "))
