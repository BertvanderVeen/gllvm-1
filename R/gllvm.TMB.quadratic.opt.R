...
gllvm.TMB.quadratic.opt <- function(y, X = NULL, formula = NULL, num.lv = 2, family = "poisson",
                                ...
                                Lambda.start=c(0.1,0.5), jitter.var=0, ridge=ridge, ridge.quadratic = ridge.quadratic, start.method=start.method, par.scale=par.scale) {
  ignore.u=FALSE
  n <- dim(y)[1]
  p <- dim(y)[2]
  ...
  while(n.i <= n.init){
    ...
    if (is.null(start.params)) {
      ...
      lvs <- NULL
        ...
        if(!is.null(X)){
          ...
          optima <- -lambdas/(2*lambda2)
          tolerances <- 1/(sqrt(-2*lambda2))
        }else if(is.null(X)){
          lambdas <- fit$params[,(ncol(fit$params) - num.lv*2 + 1):(ncol(fit$params)-num.lv)]
    ...
    b <- NULL; if(!is.null(X)) b <- matrix(betas, ncol(X), p,byrow = TRUE)
      ...
      optima <- optima[lower.tri(optima,diag = TRUE)]
    ...
    se <- NULL
      ...
      if(row.eff=="random"){
        if(ridge==T){
          ...
          objr <- TMB::MakeADFun(
            ...
            objr <- TMB::MakeADFun(
              data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,model=0,random=1, ridge=1, ridge_quadratic=1), silent=TRUE,
              parameters = list(r0 = matrix(r0), b = rbind(a,b), B = matrix(0),opt = optima, tol = t(tolerances), u = u,lg_phi=log(phi),log_sigma=log(sigma),Au=Au,lg_Ar=log(Ar),zeta=zeta, lg_gamma=rep(0,num.lv), lg_gamma2=rep(0,num.lv)),
              inner.control=list(mgcmax = 1e+200,maxit = maxit),
              DLL = "gllvm3")
          }
       ...
       if(ridge==T){
         if(ridge.quadratic==F){
           objr <- TMB::MakeADFun(
             data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,model=0,random=0, ridge=1, ridge_quadratic=0), silent=TRUE,
             parameters = list(r0=matrix(r0), b = rbind(a,b),B=matrix(0),opt = optima, tol = t(tolerances), u = u,lg_phi=log(phi),log_sigma=0,Au=Au,lg_Ar=log(Ar),zeta=zeta,lg_gamma=rep(0,num.lv), lg_gamma2=rep(0,num.lv)),
             inner.control=list(mgcmax = 1e+200,maxit = maxit),
             DLL = "gllvm3")##GLLVM
         }else{
           objr <- TMB::MakeADFun(
             data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,model=0,random=0, ridge=1, ridge_quadratic=1), silent=TRUE,
             parameters = list(r0=matrix(r0), b = rbind(a,b),B=matrix(0),opt = optima, tol = t(tolerances), u = u,lg_phi=log(phi),log_sigma=0,Au=Au,lg_Ar=log(Ar),zeta=zeta,lg_gamma=rep(0,num.lv), lg_gamma2=rep(0,num.lv)),
             inner.control=list(mgcmax = 1e+200,maxit = maxit),
             DLL = "gllvm3")##GLLVM
         }
      ...
      if(optimizer=="optim") {
        ...
        parscale[names(objr$par)=="opt"|names(objr$par)=="tol"]<-par.scale
        ...
        if(row.eff == "random"){
          if(ridge==T){
            if(ridge.quadratic==F){
              objr <- TMB::MakeADFun(
                data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,model=0,random=1, ridge=1, ridge_quadratic=0), silent=TRUE,
                parameters = list(r0=r1, b = b1,B=matrix(0),opt = optima, tol = tolerances, u = u1,lg_phi=lg_phi1,log_sigma=log_sigma1,Au=Au1,lg_Ar=lg_Ar1,zeta=zeta, lg_gamma=lg_gamma, lg_gamma2=lg_gamma2), #log(phi)
                inner.control=list(mgcmax = 1e+200,maxit = maxit),
                DLL = "gllvm3")
            }else{
              objr <- TMB::MakeADFun(
          ...
          if(ridge==T){
            if(ridge.quadratic==F){
              objr <- TMB::MakeADFun(
                data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,model=0,random=0, ridge=1, ridge_quadratic=0), silent=TRUE,
                parameters = list(r0=r1, b = b1,B=matrix(0),opt = optima, tol = tolerances, u = u1,lg_phi=lg_phi1,log_sigma=0,Au=Au1,lg_Ar=lg_Ar1,zeta=zeta, lg_gamma=lg_gamma, lg_gamma2=lg_gamma2), #log(phi)
                inner.control=list(mgcmax = 1e+200,maxit = maxit),
                DLL = "gllvm3")#GLLVM#
            }else{
              objr <- TMB::MakeADFun(
  ...
  tr<-try({
    if(sd.errors && !is.infinite(out$logL)) {
      ...
      sebetaM <- matrix(se[1:((num.X+1)*p)],p,num.X+1,byrow=TRUE);  se <- se[-(1:((num.X+1)*p))]
        ...
        #still need to fix this, these are uncertainties of optima and tolerances!
        #Should also report optima and tolerances instead of lambdas higher
