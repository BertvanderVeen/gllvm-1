...
gllvm.TMB.trait.quadratic <- function(y, X = NULL, TR = NULL, formula = NULL, num.lv = 2, family = "poisson", Lambda.struc = "unstructured",
  ...
  } ## Prevents it going to 'unstructured' loops and causing chaos
  trial.size <- 1
  ...
  out <- list(
    y = y, X = X1, TR = TR1, num.lv = num.lv, row.eff = row.eff, logL = Inf, family = family, offset = offset, randomX = randomX,
  ...
  while (n.i <= n.init) {
    ...
    old.logL <- out$logL
    if (starting.val != "lingllvm") {
      res <- start.values.gllvm.TMB.quadratic(
        y = y, X = X1, TR = TR1, family = family, offset = offset, trial.size = trial.size,
    ...
    if (is.null(start.params)) {
      ...
      if (dim(start.params$y) == dim(y) && is.null(X) == is.null(start.params$X) && is.null(T) == is.null(start.params$TR) &&
        ...
        fourth <- inter <- NULL
        if (!is.null(TR)) {
          inter <- start.params$params$fourth
        } # let's treat this as a vector (vec(B'))'
    ...
    if (row.eff == "random") {
      ...
      if (ridge == T) {
        if (ridge.quadraitc == F) {
          objr <- TMB::MakeADFun(data = list(
            y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
            model = 1, random = 1, ridge = 1, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
          ), silent = TRUE, parameters = list(
            r0 = matrix(r0), b = rbind(a),
            B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = log(sigma), Au = Au,
            lg_Ar = log(Ar), zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
          ), inner.control = list(
            mgcmax = 1e+200,
            maxit = maxit
          ), DLL = "gllvm2")
          ...
          objr <- TMB::MakeADFun(data = list(
            y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
            model = 1, random = 1, ridge = 1, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
          ), silent = TRUE, parameters = list(
            r0 = matrix(r0), b = rbind(a),
            B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = log(sigma), Au = Au,
            lg_Ar = log(Ar), zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
        ...
        if (ridge.quadratic == F) {
          objr <- TMB::MakeADFun(data = list(
            y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
            model = 1, random = 1, ridge = 0, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
          ), silent = TRUE, parameters = list(
            r0 = matrix(r0), b = rbind(a),
            B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = log(sigma), Au = Au,
            lg_Ar = log(Ar), zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
          ...
          objr <- TMB::MakeADFun(data = list(
            y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
            model = 1, random = 1, ridge = 0, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
          ), silent = TRUE, parameters = list(
            r0 = matrix(r0), b = rbind(a),
            B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = log(sigma), Au = Au,
            lg_Ar = log(Ar), zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
      ...
      if (ridge == T) {
        if (ridge.quadratic == F) {
          objr <- TMB::MakeADFun(
            data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
              model = 1, random = 0, ridge = 1, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = matrix(r0), b = rbind(a),
              B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = 0, Au = Au, lg_Ar = log(Ar),
              zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
          ...
          objr <- TMB::MakeADFun(
            data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
              model = 1, random = 0, ridge = 1, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = matrix(r0), b = rbind(a),
              B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = 0, Au = Au, lg_Ar = log(Ar),
              zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
        ...
        if (ridge.quadratic == F) {
          objr <- TMB::MakeADFun(
            data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
              model = 1, random = 0, ridge = 0, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = matrix(r0), b = rbind(a),
              B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = 0, Au = Au, lg_Ar = log(Ar),
              zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
          ...
          objr <- TMB::MakeADFun(
            data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn, method = 0,
              model = 1, random = 0, ridge = 0, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = matrix(r0), b = rbind(a),
              B = matrix(B), lambda = theta, lambda2 = t(theta2), u = u, lg_phi = log(phi), log_sigma = 0, Au = Au, lg_Ar = log(Ar),
              zeta = zeta, lg_gamma = rep(0, num.lv), lg_gamma2 = rep(0, num.lv)
    ...
    if (diag.iter > 0 && Lambda.struc == "unstructured" && row.eff == "random" && is.null(randomX)) {
      ...
      if (row.eff == "random") {
        if (ridge == T) {
          if (ridge.quadratic == F) {
            objr <- TMB::MakeADFun(data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
              method = 0, model = 1, random = 1, ridge = 1, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = r1,
              b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = lg_sigma1, Au = Au1,
              lg_Ar = Ar1, zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
            ), inner.control = list(
              mgcmax = 1e+200,
              maxit = 1000
            ), DLL = "gllvm2")
            ...
            objr <- TMB::MakeADFun(data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
              method = 0, model = 1, random = 1, ridge = 1, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = r1,
              b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = lg_sigma1, Au = Au1,
              lg_Ar = Ar1, zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
          ...
          if (ridge.quadratic == F) {
            objr <- TMB::MakeADFun(data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
              method = 0, model = 1, random = 1, ridge = 0, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = r1,
              b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = lg_sigma1, Au = Au1,
              lg_Ar = Ar1, zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
            ...
            objr <- TMB::MakeADFun(data = list(
              y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
              method = 0, model = 1, random = 1, ridge = 0, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
            ), silent = TRUE, parameters = list(
              r0 = r1,
              b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = lg_sigma1, Au = Au1,
              lg_Ar = Ar1, zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
        ...
        if (ridge == T) {
          if (ridge.quadratic == F) {
            objr <- TMB::MakeADFun(
              data = list(
                y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
                method = 0, model = 1, random = 0, ridge = 1, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
              ), silent = TRUE, parameters = list(
                r0 = r1,
                b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = 0, Au = Au1, lg_Ar = Ar1,
                zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
              ), inner.control = list(mgcmax = 1e+200, maxit = 1000),
              DLL = "gllvm2"
            )
          } else {
            objr <- TMB::MakeADFun(
              data = list(
                y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
                method = 0, model = 1, random = 0, ridge = 1, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
              ), silent = TRUE, parameters = list(
                r0 = r1,
                b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = 0, Au = Au1, lg_Ar = Ar1,
                zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
          ...
          if (ridge.quadratic == F) {
            objr <- TMB::MakeADFun(
              data = list(
                y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
                method = 0, model = 1, random = 0, ridge = 0, ridge_quadratic = 0, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
              ), silent = TRUE, parameters = list(
                r0 = r1,
                b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = 0, Au = Au1, lg_Ar = Ar1,
                zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
            ...
            objr <- TMB::MakeADFun(
              data = list(
                y = y, x = Xd, xr = xr, offset = offset, num_lv = num.lv, family = familyn,
                method = 0, model = 1, random = 0, ridge = 0, ridge_quadratic = 1, trace = as.integer(trace2), zetastruc = ifelse(zeta.struc == "species", 1, 0)
              ), silent = TRUE, parameters = list(
                r0 = r1,
                b = b1, B = B1, lambda = lambda1, lambda2 = lambda2, u = u1, lg_phi = lg_phi1, log_sigma = 0, Au = Au1, lg_Ar = Ar1,
                zeta = zeta, lg_gamma = lg_gamma, lg_gamma2 = lg_gamma2
