  ...
  while(n.i <= n.init){
    ...
    if(row.eff=="random"){# || !is.null(randomX)
          ...
          objr <- TMB::MakeADFun(
            ...
            parameters = list(r0=matrix(r0), b = rbind(a),B=matrix(B),lambda = theta, lambda2=t(theta2), u = u,lg_phi=log(phi),log_sigma=log(sigma),Au=Au,lg_Ar=log(Ar), zeta=zeta),
          ...
          objr <- TMB::MakeADFun(
            ...
            parameters = list(r0=matrix(r0), b = rbind(a),B=matrix(B),lambda = theta, lambda2=t(theta2), u = u,lg_phi=log(phi),log_sigma=0,Au=Au,lg_Ar=log(Ar), zeta=zeta),
    ...
    if(diag.iter>0 && Lambda.struc=="unstructured" && row.eff=="random" && is.null(randomX)){
      ...
      if(row.eff=="random"){
          ...
          objr <- TMB::MakeADFun(
            data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,method=0,model=1,random=1), silent=!trace,
            parameters = list(r0=r1, b = b1,B=B1,lambda = lambda1, lambda2=lambda2, u = u1,lg_phi=lg_phi1,log_sigma=lg_sigma1,Au=Au1,lg_Ar=Ar1, zeta=zeta),
            inner.control=list(mgcmax = 1e+200,maxit = 1000),
            DLL = "gllvm2")
        ...
        objr <- TMB::MakeADFun(
          data = list(y = y, x = Xd,xr=xr,offset=offset, num_lv = num.lv,family=familyn,extra=extra,method=0,model=1,random=0), silent=!trace,
          parameters = list(r0=r1, b = b1,B=B1,lambda = lambda1, lambda2=lambda2, u = u1,lg_phi=lg_phi1,log_sigma=0,Au=Au1,lg_Ar=Ar1, zeta=zeta),
          inner.control=list(mgcmax = 1e+200,maxit = 1000),
          DLL = "gllvm2")
      }
