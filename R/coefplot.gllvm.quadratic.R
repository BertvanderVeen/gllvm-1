    ...
    if (is.null(object$TR)) {
        ...
        for (i in 1:k) {
            ...
            if (!is.null(xlim.list[[i]])) {
                ...
                plot(x = Xc, y = At.y, yaxt = "n", ylab = "", col = col.seq, xlab = cnames[i], xlim = c(min(lower), max(upper)), pch = "x", 
                  cex.lab = 1.3, ...)
            }
        ...
        if (!is.null(xlim.list)) {
            ...
            plot(x = Xc, y = At.y, yaxt = "n", ylab = "", col = col.seq, xlab = "Coefficients", xlim = c(min(lower), max(upper)), 
                pch = "x", cex.lab = 1.3, ...)
        }
        segments(x0 = lower, y0 = At.y, x1 = upper, y1 = At.y, col = col.seq)
        abline(v = 0, lty = 1)
