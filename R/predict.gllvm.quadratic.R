...
predict.gllvm.quadratic <- function(object, newX = NULL, newTR = NULL, newLV = NULL, type = "link", LVonly = FALSE, which.lvs = 1:object$num.lv,intercept=T, 
    ...) {
    newdata <- newX
    p <- ncol(object$y)
    ...
    if (object$num.lv > 0) {
        ...
        if (is.null(newLV) && is.null(newdata) && is.null(newTR)) 
            eta <- eta + object$lvs[, which.lvs, drop = F] %*% t(theta[, -c(object$num.lv + object$num.lv), drop = F][, which.lvs, 
                drop = F]) + object$lvs[, which.lvs, drop = F]^2 %*% t(theta[, -c(1:object$num.lv), drop = F][, which.lvs, drop = F])
        if (!is.null(newLV)) {
            ...
            eta <- eta + lvs[, , drop = F] %*% t(theta[, -c(object$num.lv + object$num.lv), drop = F][, which.lvs, drop = F]) + lvs[, 
                , drop = F]^2 %*% t(theta[, -c(1:object$num.lv), drop = F][, which.lvs, drop = F])
        }
    }
